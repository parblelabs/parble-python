...
def test_post_path(sdk, tmp_path, text, dummy_file_attributes):
...
def test_post_file_default_content_type(sdk, text, dummy_file_attributes):
...
def test_post_file_override_content_type(sdk, text, dummy_file_attributes):
    buf = BytesIO(text.encode())
    with patch("parble.resources.files.FilesResource.post") as m:
        m.return_value = dummy_file_attributes
